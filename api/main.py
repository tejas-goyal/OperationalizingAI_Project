...
app = FastAPI(title="Crypto Volatility API", version="1.2.0")

Instrumentator().instrument(app).expose(app, endpoint="/metrics")

LAST_EVENT_TS = Gauge(
    "last_event_timestamp_seconds",
    "Unix timestamp of the last processed prediction event",
)
...
@app.post("/predict", response_model=RowsResponse)
def predict_rows(request: RowsRequest):
    """Batch prediction: accepts {"rows": [...]}, returns {"scores": [...]}."""
...
@app.post("/predict/legacy", response_model=PredictResponse)
def predict_legacy(request: PredictRequest):
    """Legacy single-row prediction: accepts {"features": {...}}."""
    global predict_count, spike_count, error_count
